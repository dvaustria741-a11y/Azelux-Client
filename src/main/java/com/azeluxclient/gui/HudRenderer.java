...
public class HudRenderer {
    ...
    private static final int COL_BG     = 0xBB0A0A1A;
    ...
    private static void onHud(DrawContext ctx, RenderTickCounter tc) {
        ...
        // ── Active module list (top-right, like Lunar Client) ─────────────
        ...
        int leftY = 28; // below watermark + coords
        ...
        if (dirHUD != null && dirHUD.isEnabled()) {
            String face = dirHUD.getFacingLabel(mc) + " - " + dirHUD.getFacingFull(mc);
            String txt = "\u2316 " + face;
        ...
        if (chunkMod != null && chunkMod.isEnabled()) {
            ...
            String txt = "Chunk: " + chunk[0] + ", " + chunk[1];
            ctx.drawText(tr, txt, 4, sh - 26, COL_DIM, false);
        }
    }

    private static int renderArmorHUD(DrawContext ctx, MinecraftClient mc, ArmorHUD mod, int y) {
